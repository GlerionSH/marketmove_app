...
class _VentasListScreenState extends State<VentasListScreen> {
  ...
  Future<void> _loadVentas() async {
    ...
    try {
      ...
      final ventas = await DataService.getVentas();
      setState(() {
        _ventas = ventas;
        _loading = false;
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          await context.push('/ventas/nueva');
          _loadVentas();
        },
        ...
        children: [
          if (_isSuperAdmin)
            Padding(
              ...
              child: GlobalViewToggle(onChanged: _loadVentas),
            ),
  ...
  Future<void> _exportToPdf(AppLocalizations t) async {
    try {
      ...
      final bytes = await ExportService.exportSalesToPDF(
        sales: _ventas,
        locale: locale,
        title: t.sales,
      ...
      await ExportService.sharePdf(bytes, 'ventas_${DateTime.now().millisecondsSinceEpoch}.pdf');
  ...
  Future<void> _exportToExcel(AppLocalizations t) async {
    try {
      ...
      final locale = Localizations.localeOf(context).languageCode;
      final bytes = await ExportService.exportSalesToExcel(
        sales: _ventas,
        locale: locale,
      );
      ...
      await ExportService.saveAndOpenFile(
        bytes: bytes,
        fileName: 'ventas_${DateTime.now().millisecondsSinceEpoch}.xlsx',
      );
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return AnimatedListItem(
                ...
                child: CustomCard(
                  onTap: () async {
                    await context.push('/ventas/${venta.id}');
                    _loadVentas();
                  },
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              '\$${venta.importe.toStringAsFixed(2)}',
                            ...
                            Text(
                              '${t.quantity}: ${venta.cantidad} | ${venta.fecha != null ? dateFormat.format(venta.fecha!) : "-"}',
                              style: TextStyle(
                                color: colorScheme.onSurfaceVariant,
                                fontSize: 14,
                              ),
